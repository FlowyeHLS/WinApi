...
//CONST INT g_i_BUTTON_SPACE_DOUBLE = g_i_BUTTON_SPACE *2;
...
INT WINAPI WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		{
			if (input == FALSE)SendMessage(hEditDisplay, WM_SETTEXT, 0, (LPARAM)szDisplay);
		...
		if (LOWORD(wParam) >= IDC_BUTTON_PLUS && LOWORD(wParam) <= IDC_BUTTON_SLASH)
		{
			...
			input = FALSE;
			SendMessage(hwnd, WM_COMMAND, (WPARAM)IDC_BUTTON_EQUAL, 0);// Выролняем предыдущую операцию
			operation = wParam;//и только после этого запоминаем введенную операцию 
	...
	case WM_KEYDOWN:
	{
		...
		if (GetKeyState(VK_SHIFT) < 0)
			if (wParam == 0x38)
			{
				...
				SendMessage(GetDlgItem(hwnd,IDC_BUTTON_ASTER),BM_GETSTATE,TRUE, 0);
			}
			else
				if (wParam >= '0' && wParam <= '9')
				{
		...
		case VK_DECIMAL:	SendMessage(GetDlgItem(hwnd, IDC_BUTTON_PLUS), BM_SETSTATE, TRUE, 0); break;
		...
		case VK_OEM_2:		SendMessage(GetDlgItem(hwnd, IDC_BUTTON_PLUS), BM_SETSTATE, TRUE, 0); break;
